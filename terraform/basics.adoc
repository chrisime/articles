...
In `variables.tf` ist eine Variable für den Image Namen für Docker hinterlegt. Der Default-Wert ist hier mit angegeben, kann aber wie weiter oben schon gesagt, auch weggelassen werden. In diesem Fall müsste der Entwickler jedes Mal das Image angeben.

Für die Ausgabe habe ich eine Kombination aus Image Namen und dessen Digest definiert. Wenngleich etwas künstlich anmutend möchte ich damit die Mächtigkeit von _Output_ zeigen.
